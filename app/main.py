...
with col_statistics:
    ...
    # TODO: Order barplots by count

    st.subheader("Most used words")
    words_pipeline = [{"$match": mongo_query}, {"$unwind": "$words_count"}, {"$group" : {"_id": "$words_count.word", "count": {"$sum": {"$toInt": "$words_count.count"}}}}, {"$sort": {"count": -1}}, {"$limit": 50}]
